...
class TutorLMS {
    ...
    public function is_user_enrolled(int $user_id, int $course_id): bool {
        if (function_exists('tutor_utils')) {
            ...
            // La función is_enrolled() de Tutor LMS devuelve un objeto en caso de éxito y false si no.
            // Para cumplir con la declaración de retorno `: bool`, convertimos (casteamos) el
            // resultado a un booleano explícitamente. Un objeto se evaluará como `true`.
    ...
    public function cancel_enrollment(int $user_id, int $course_id): bool {
        if (function_exists('tutor_utils')) {
            // --- INICIO DE LA SOLUCIÓN ---
            // Al igual que `is_user_enrolled`, la función `cancel_enrolment` puede no devolver
            // un booleano estricto. La convertimos para asegurar la compatibilidad.
            return (bool) tutor_utils()->cancel_enrolment($course_id, $user_id);
            // --- FIN DE LA SOLUCIÓN ---
        }
        return false;
    }
